    ...
    def handle_protect(self) -> None:
        try:
            ...
            if not SECURITY_EXE.exists():
                raise FileNotFoundError(f"Missing executable: {SECURITY_EXE.name}. Please compile the CUDA files first.")
    ...
    def handle_detect(self) -> None:
        try:
            ...
            if not DETECTOR_EXE.exists():
                raise FileNotFoundError(f"Missing executable: {DETECTOR_EXE.name}. Please compile the CUDA files first.")
